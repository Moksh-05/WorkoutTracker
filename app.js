...
async function saveCustomExercise() {
  ...
  const { data, error } = await sb.from("exercises").insert({ name, category, subtype }).select().single();
...
async function getOrCreateWorkoutDay(isRestDay = false) {
  ...
  const { data, error } = await sb.from("workout_days").insert({ date, is_rest_day: isRestDay }).select().single();
...
async function addSet() {
  ...
  if (error) { queueOffline("logged_sets", row); } else { setStatus("Set saved."); }
...
function renderTodaysSets() {
  ...
  todaysSets.forEach(s => {
    ...
    body.innerHTML += `<tr><td>${s.exercise_name}</td><td>${weightOrDuration}</td><td>${s.reps ?? "-"}</td><td>${flags || "-"}</td></tr>`;
  });
}
...
async function renderBodyWeightChart() {
  ...
  if (!data) return;
  drawChart("bodyweight-chart", { labels: data.map(d => d.date), values: data.map(d => d.weight_lbs) }, "Body Weight (lbs)");
}
